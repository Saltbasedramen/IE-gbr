 ...
 *
 */
const wcpa_run_api = async (api_url, data = {}, method = "GET") => {
    var $fb = jQuery(document.getElementById("wcpa_editor"));
    let returnData = {
        error: true,
        data: [],
        msg: "",
    ...
    await jQuery.ajax({
        type: method,
        url: `${wcpa_backend_vars.rest_base_url}/${api_url}/`,
        data: data,
        dataType: "json",
        ...
        error: function (xhr) {
            ...
            $fb.append(
                '<div class="wcpa_form_init_error">' + xhr.responseJSON.message
                    ? xhr.responseJSON.message
                    : form_builder_i18n.error_checkout_fields + "</div>"
            );
            $fb.find(".wcpa_form_preloader").remove();
        },
    });
    return returnData;
};
...
jQuery(async function ($) {
    ...
    if ($fb.length) {
        $fb.append(
            '<div class="wcpa_form_preloader"><img src="' +
                wcpa_backend_vars.plugin_path +
                'assets/img/preloader.svg"></div>'
        );
        let chfields = [];
        if(wcpa_backend_vars.enable_map_to_checkout) {
            chfields = await wcpa_run_api("get_checkout_fields");
        }
