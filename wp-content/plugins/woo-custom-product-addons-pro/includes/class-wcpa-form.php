...
//dividing with 60 will give the execution time in minutes otherwise seconds
    ...
    public function validate_form_data($product_id = false, $qty = 1, $variation_id = false)
    {
        $this->get_forms_by_product($product_id);
...
//                                    $unixTimestamp = strtotime($sub_data['value']);
    ...
    public function render($after_acb = false, $is_rest = false)
    {
        ...
        echo '</div>';
        if (isset($this->settings['show_validation_error_box']) && $this->settings['show_validation_error_box'] === true) {
            $hook = get_wcpa_display_hook('validation_summary');
            add_action($hook[0], array($this, 'validation_summary_box'), $hook[1]);
        }
        if ($has_price) {
        ...
        do_action('wcpa_validation_summary_box');
    ...
    public function render_text($v)
    {
        ...
        if (isset($v->label) && $v->label) {
            $validation_array['label'] = trim($v->label);
        } else if(isset($v->name) && $v->name) {
            $validation_array['label'] = trim($v->name);
        }
    ...
    public function label_section($v)
    {
        ...
        if ((!isset($v->desc_type) || !$v->desc_type) && (isset($v->description) || (isset($v->helptext_change) && $v->helptext_change))) {
            echo '<span class="wcpa_helptext">' . (isset($v->description) ? $v->description : '' );
            echo '</span>';
        }
    }
    ...
    public function render_radio($v)
    {
        ...
        if (isset($v->helptext_change) && $v->helptext_change ) {
            $className .= 'wcpa_change_help_text ';
        }
    ...
    public function render_file($v)
    {
        ...
        $multipleClass = '';
        $DropContainerClass = 'wcpa_single_upload';
        if ($multiple_enabled) {
    ...
    public function render_statictext($v)
    {
        ...
        if (!(isset($v->inline) && $v->inline)) {
            ...
            echo '<span class="wcpa_' . $v->type . '_label_separator"> : </span>' . $value;
...
//                    $is_selected = ($default_value === (string) $k) ? true : false;
    ...
    public function validation_summary_box()
    {
        echo '<div class="wcpa_validation_summary"><ul>';
        echo ' </ul></div>';

        $hook = get_wcpa_display_hook('validation_summary');
        remove_action($hook[0], array($this, 'validation_summary_box'), $hook[1]);
    }
