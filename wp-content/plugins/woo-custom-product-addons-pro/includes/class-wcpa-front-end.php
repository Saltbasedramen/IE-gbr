...
class WCPA_Front_End extends WCPA_Order_Meta
{
    ...
    public function __construct($file = '', $version = '1.0.0')
    {
        ...
        if ($this->check_woocommerce_active()) {
            ...
            add_filter('woocommerce_checkout_get_value', array($this, 'set_checkout_field_input_value_default'), 9, 2);
            ...
            add_filter('woocommerce_loop_add_to_cart_link', array($this, 'add_to_cart_button_customization'), 20, 3);
    ...
    public static function get_cart_error($product_id = false)
    {
        if (!$product_id) {
    ...
    public static function set_cart_error($product_id, $status)
    {
    ...
    public function rest_api_init()
    {
        ...
        register_rest_route( 'wcpa/v1', '/get_checkout_fields/', array(
            'methods' => 'GET',
    ...
    public function get_checkout_fields() {
        ...
        ob_end_clean(); // Added because an error in customer website due to appending some extra values on this return
        return new WP_REST_Response($checkout_key_val_pair, 200);
    }
...
//    }
    ...
    public function add_to_cart_validation($passed, $product_id, $qty = 1, $variation_id = false)
    {
        ...
        $form = new WCPA_Form();
        if (
            (($pid = wp_get_post_parent_id($product_id)) != 0) && 
            ($variation_id == false)
        ) {
            $variation_id = $product_id;
            $product_id = $pid;
        }
        $passed = $form->validate_form_data($product_id, $qty, $variation_id);
...
//        }
        ...
        $wcpa_global_vars['disable_validation_scroll'] = wcpa_get_option('wcpa_disable_validation_scroll', false);
        $wcpa_global_vars['show_validation_box'] = wcpa_get_option('wcpa_show_val_error_box', false);
