...
if (!function_exists('green_planet_system_fonts_list')) {
	function green_planet_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('green_planet_text_decoration_list')) {
	function green_planet_text_decoration_list() {
		$list = array( 
...
if (!function_exists('green_planet_custom_color_schemes_list')) {
	function green_planet_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'green-planet'), 
			'second' => esc_html__('Custom 2', 'green-planet'), 
			'third' => esc_html__('Custom 3', 'green-planet') 
		);
		
		
		return apply_filters('green_planet_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('green_planet_locate_template')) {
	function green_planet_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_green-planet_theme_style') ? get_option('cmsmasters_green-planet_theme_style') : ''));
...
if (!function_exists('green_planet_all_theme_styles')) {
	function green_planet_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('green_planet_all_color_schemes_list')) {
	function green_planet_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'green-planet'), 
			'header' => 		esc_html__('Header', 'green-planet'), 
			'navigation' => 	esc_html__('Navigation', 'green-planet'), 
			'header_top' => 	esc_html__('Header Top', 'green-planet'), 
			'footer' => 		esc_html__('Footer', 'green-planet') 
		);
		
		
		$out = array_merge($list, green_planet_custom_color_schemes_list());
...
if (CMSMASTERS_LEARNPRESS) {
	...
	function green_planet_change_template_for_learnpress() {
...
if (!function_exists('green_planet_load_theme_textdomain')) {
	function green_planet_load_theme_textdomain() {
		load_theme_textdomain('green-planet', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'green_planet_load_theme_textdomain')) {
	add_action('after_setup_theme', 'green_planet_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('green_planet_theme_activation')) {
	function green_planet_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'green-planet') {
			add_option('cmsmasters_active_theme', 'green-planet', '', 'yes');
...
add_action('after_switch_theme', 'green_planet_theme_activation');



// Framework Deactivation
if (!function_exists('green_planet_theme_deactivation')) {
	function green_planet_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'green_planet_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('green_planet_plugin_activation')) {
	function green_planet_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'green_planet_plugin_activation', 10, 2);


if (!function_exists('green_planet_plugin_activation_regenerate')) {
	function green_planet_plugin_activation_regenerate() {
...
add_action('init', 'green_planet_plugin_activation_regenerate');


function green_planet_run_reinit_import_options($post_id, $key, $value) {
	if (!get_post_meta($post_id, 'cmsmasters_heading', true)) {
		$custom_post_meta_fields = green_planet_get_custom_all_meta_fields();
...
add_action('import_post_meta', 'green_planet_run_reinit_import_options', 10, 3);
