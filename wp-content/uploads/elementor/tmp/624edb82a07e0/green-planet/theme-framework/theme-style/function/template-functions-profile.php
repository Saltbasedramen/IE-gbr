...
function green_planet_profile_heading($cmsmasters_id, $tag = 'h1', $sub_title = false, $tag_sub = 'h3', $show = true) { 
...
function green_planet_profile_title_nolink($cmsmasters_id, $tag = 'h1', $sub_title = false, $tag_sub = 'h5', $show = true) { 
...
function green_planet_get_profile_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['green-planet' . '_profile_post_cat']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'green-planet') . ':' . '</div>' . 
...
function green_planet_get_profile_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['green-planet' . '_profile_post_like']) {
			$out = '<div class="profile_details_item">' . 
				'<div class="profile_details_item_title">' . esc_html__('Likes', 'green-planet') . ':' . '</div>' . 
...
function green_planet_get_profile_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = green_planet_get_comments('cmsmasters_profile_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = green_planet_get_global_options();
			
			if ($cmsmasters_option['green-planet' . '_profile_post_comment']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Comments', 'green-planet') . ':' . '</div>' . 
					'<div class="profile_details_item_desc details_item_desc_comments">' . 
						green_planet_get_comments('cmsmasters_profile_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function green_planet_get_profile_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function green_planet_profile_social_icons($social_icons, $shortcode_id = false, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
...
function green_planet_profile_open_social_icons($social_icons, $profile_id = false, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
		$out = '<div class="profile_social_icons">';
...
function green_planet_profile_open_style_social_icons() {
	...
	if ($social_icons != '') {
		...
		if ($social_icon_styles != '') {
			wp_add_inline_style('green-planet-style', $social_icon_styles);
		}
	}
}

add_action('wp_enqueue_scripts', 'green_planet_profile_open_style_social_icons');
