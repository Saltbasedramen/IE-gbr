...
function green_planet_prev_next_posts($order_cat = false, $tax_name = 'category') {
...
function green_planet_sharing_box($title_box = false, $tag = 'h3') {
...
function green_planet_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'green-planet') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function green_planet_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
...
function green_planet_author_avatar($template_type = 'page') {
...
function green_planet_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'green-planet') . '</' . esc_html($tag) . '>' . "\n" .
