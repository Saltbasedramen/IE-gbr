...
if (
	$cmsmasters_option['green-planet' . '_portfolio_project_like'] || 
	$cmsmasters_option['green-planet' . '_portfolio_project_date'] || 
	$cmsmasters_option['green-planet' . '_portfolio_project_cat'] || 
	$cmsmasters_option['green-planet' . '_portfolio_project_comment'] || 
	$cmsmasters_option['green-planet' . '_portfolio_project_author'] || 
	$cmsmasters_option['green-planet' . '_portfolio_project_tag'] || 
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_project'); ?>>
	...
	if ($cmsmasters_project_title == 'true') {
		echo '<header class="cmsmasters_project_header entry-header">';
			green_planet_project_title_nolink(get_the_ID(), 'h1');
		echo '</header>';
	}
	...
	echo '<div class="project_content' . (($project_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if ($cmsmasters_post_format == 'gallery') {
			...
			green_planet_post_type_gallery(get_the_ID(), $cmsmasters_project_images, $cmsmasters_project_columns, 'cmsmasters-project-thumb', 'cmsmasters-project-full-thumb');
			...
			green_planet_post_type_video(get_the_ID(), $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, 'cmsmasters-project-full-thumb');
			...
			if ($cmsmasters_project_image_show == 'true' && $cmsmasters_project_images[0] == '') {
				...
				green_planet_post_type_slider(get_the_ID(), $cmsmasters_project_images, 'cmsmasters-full-masonry-thumb');
			}
		}
			...
			echo '<div class="project_sidebar">';
				...
				if ($project_details == 'true') {
					echo '<div class="project_details entry-meta">' . 
						...
						green_planet_get_project_likes('post');
						
						green_planet_get_project_comments('post');
						
						green_planet_get_project_author('post');
						
						green_planet_get_project_date('post');
						
						green_planet_get_project_category(get_the_ID(), 'pj-categs', 'post');
						
						green_planet_get_project_tags(get_the_ID(), 'pj-tags');
						
						green_planet_get_project_features('details', $cmsmasters_project_features, false, 'h3', true);
						
						green_planet_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
						
					echo '</div>';
				}
				
				
				green_planet_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h3', true);
				
				green_planet_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h3', true);
				
				green_planet_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h3', true);
...
if ($cmsmasters_project_sharing_box == 'true') {
	green_planet_sharing_box(esc_html__('Like this project?', 'green-planet'), 'h2');
}


if ($cmsmasters_option['green-planet' . '_portfolio_project_nav_box']) {
	$order_cat = (isset($cmsmasters_option['green-planet' . '_portfolio_project_nav_order_cat']) ? $cmsmasters_option['green-planet' . '_portfolio_project_nav_order_cat'] : false);
	
	green_planet_prev_next_posts($order_cat, 'pj-categs');
}


if ($cmsmasters_project_author_box == 'true') {
	green_planet_author_box(esc_html__('About author', 'green-planet'), 'h2', 'h4');
}
...
if ($cmsmasters_project_more_posts != 'hide') {
	green_planet_related( 
		'h2', 
		esc_html__('More projects', 'green-planet'), 
		esc_html__('No projects found', 'green-planet'), 
		$cmsmasters_project_more_posts, 
		$tgsarray, 
		$cmsmasters_option['green-planet' . '_portfolio_more_projects_count'], 
		$cmsmasters_option['green-planet' . '_portfolio_more_projects_pause'], 
		'project', 
		'pj-tags' 
	);
}
