...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_profile_horizontal ' . $columns_num); ?>>
	...
	echo '<div class="cmsmasters_img_social_wrap' . (has_post_thumbnail() ? '' : ' no_image') . '">' . 
		'<div class="cmsmasters_img_social_inner">';
			...
			if (has_post_thumbnail()) {
				green_planet_thumb(get_the_ID(), 'cmsmasters-square-thumb', true, false, false, false, false);
			} else {
			...
			green_planet_profile_social_icons($cmsmasters_profile_social, $profile_id);
	...
	echo '<div class="profile_inner">';
		...
		green_planet_profile_heading(get_the_ID(), 'h2', $cmsmasters_profile_subtitle, 'h6');
		
		green_planet_profile_exc_cont();
