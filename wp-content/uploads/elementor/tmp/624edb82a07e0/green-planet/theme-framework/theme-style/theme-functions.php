...
function green_planet_register_theme_js_scripts() {
	...
	$cmsmasters_localize_array = array(
		...
		'secondary_color' => 	$cmsmasters_option['green-planet' . '_default_secondary'] 
	);
	
	wp_localize_script('green-planet-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'green_planet_register_theme_js_scripts');



/* Scripts for Admin */
function green_planet_theme_admin_scripts() {
	$cmsmasters_option = green_planet_get_global_options();
	...
	wp_localize_script('cmsmasters-theme-settings-toggle', 'cmsmasters_theme_settings', array( 
		...
		'header_style' => $cmsmasters_option['green-planet' . '_header_styles'] 
	));
}

add_action('admin_enqueue_scripts', 'green_planet_theme_admin_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'green-planet'), 
    'footer' => 	esc_html__('Footer Navigation', 'green-planet'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'green-planet') 
));
...
function green_planet_fallback_menu($args) {
	$cmsmasters_option = green_planet_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['green-planet' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function green_planet_pagination_prev_text() {
 return 'Previous';
}

add_filter('green_planet_pagination_prev_text_filter', 'green_planet_pagination_prev_text');


/* Pagination Next Text */
function green_planet_pagination_next_text() {
 return 'Next';
}

add_filter('green_planet_pagination_next_text_filter', 'green_planet_pagination_next_text');



/* Single Project and Profile Layout */
function green_planet_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'green_planet_single_project_layout');



/* Search Form */
function green_planet_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('enter keywords', 'green-planet') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'green_planet_search_form');
