...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_puzzle ' . $cmsmasters_project_size); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		green_planet_thumb_rollover(get_the_ID(), 'full', false, $rollover, false, false, false, false, false, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target, 'cmsmasters_theme_icon_image', $cmsmasters_project_puzzle_image);
		...
		if ($title || $categories || $comments || $likes) {
			echo '<div class="project_inner">' . 
				'<div class="project_inner_cont">';
					...
					$title ? green_planet_project_heading(get_the_ID(), 'h3', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';
					
					
					if ($categories || $comments || $likes) {
						echo '<div class="cmsmasters_project_cont_info' . (($comments || $likes) ? ' enable_project_footer' : '') . ' entry-meta">';
							
							$categories ? green_planet_get_project_category(get_the_ID(), 'pj-categs', 'page') : '';
